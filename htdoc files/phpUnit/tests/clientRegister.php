<?php
     ...
     include_once 'class.php'; // to include the class.php
     /*$con = mysqli_connect("localhost", "root", "", "test");
         if(!$con)
         {
             die('Connect Error (' . mysqli_connect_errno() . ') '. mysqli_connect_error());
         }
     */ //this was to check databade connection, implement in class.php

     $user = new User();
     if($_SERVER["REQUEST_METHOD"] == "POST"){
         $register = $user-> register($_REQUEST['email'],$_REQUEST['password']);
         if($register){
             echo "Registeration done!";
         }
         else{
             echo "Entered email already exists! enter a new one.";
